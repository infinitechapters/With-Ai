...
const GenTitle = () => {
  ...
  const titleCategories =[
    'General', 'Business','Education', 'Health','Technology','Travel','Food'
  ]
...
const {getToken }= useAuth();
  ...
  const onSubmitHandler= async(e)=>{
    e.preventDefault();
    setLoading(true);
     const prompt= `Generate title for ${input} in the category ${selectedCategory.text}`;
    const {data}= await axios.post('/api/ai/gen-titles',{
      prompt},{
    ...
    <div className='h-full  overflow-y-scroll p-6 flex items-start justify-center flex-wrap gap-4 text-slate-700'>
          ...
          <form onSubmit={onSubmitHandler} className='w-full max-w-lg mr-4 p-4 bg-white rounded-lg border border-gray-200'>
            <div className='flex items-center gap-3'>
              ...
              <h1 className='text-xl font-semibold'>Generate Title</h1>
            </div>
            ...
            <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
              {titleCategories.map((item)=>(
                <span onClick={()=>setSelectedCategory(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer 
                  ${selectedCategory=== item ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300' }`} key={item} >{item}</span>
              ))}
            </div>
            <br></br>
...
<div  className='flex-1 flex justify-center items-center'>
           <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
            ...
            <p>Enter your required category and click "Generate Title" to get started</p>
           </div>
         </div>
              ):(
